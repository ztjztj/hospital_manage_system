...
def enter_edit(request,l_id):
    ...
    return render(request,'pay_money_place/edit.html',{'each_data':each_data,"enter_patient":enter_patient,"all_depart":all_depart})
...
def medicine_look(request,m_id):
    ...
    if Medicine.objects.filter(medicine_name=med_name).exists():
        ...
        return render(request,'pay_money_place/medicine-look.html',{'each_data':each_data,'s':s,'all_money':all_money})
    else:
...
def export_detail(request,o_id):
    ...
    b = [list_obj.id, list_obj.patient_name, list_obj.patient_medicine,list_obj.patient_live_day, list_obj.patient_one_enter_time]
...
def del_patient(request):
    ...
    for uid in sid:
        check.append(uid)
        Main_details.objects.filter(id=int(uid)).delete()
...
def enter_del(request):
    ...
    for uid in sid:
        check.append(uid)
        PatientOne.objects.filter(id=int(uid)).delete()
    return JsonResponse({'check': check})
...
def outer_del(request):
    ...
    for uid in sid:
        check.append(uid)
        Money.objects.filter(id=int(uid)).delete()
    return JsonResponse({'check': check})
